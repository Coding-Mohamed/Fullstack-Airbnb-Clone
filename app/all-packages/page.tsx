  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 mt-5">
      ...
      <div className="flex space-x-4 overflow-x-auto pb-4 mb-6">
        <button onClick={() => setSelectedCategory(null)} className={`px-4 py-2 rounded-full ${!selectedCategory ? "bg-[#DE8022] text-white" : "bg-gray-200"}`}>
          All
        </button>
        {categories.map((category, index) => (
          <button key={index} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 rounded-full ${selectedCategory === category ? "bg-[#DE8022] text-white" : "bg-gray-200"}`}>
            {category}
          </button>
        ))}
      </div>
